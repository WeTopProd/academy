...
export default function LoginForm() {
  ...
  >();
  const onSubmit: SubmitHandler<FormValuesEmail | FormValuesPhone> = (data) =>
    console.log(data);
  ...
  return (
    <>
      <ul className={styles.variants_picker}>
        <li
          onClick={() => setPhase((prev) => (prev = 'phone'))}
          className={phase === 'phone' ? styles.li_active : styles.li}
        >
          По номеру телефона
        </li>
        <li
          onClick={() => setPhase('email')}
          className={phase === 'email' ? styles.li_active : styles.li}
        >
          По почте
        </li>
      </ul>
